...
class FaceProcessor:
    ...
    def load_registered_faces(self):
        """Load all registered faces from JSON file to GPU"""
        try:
            with open(JSON_DB_PATH, 'r') as f:
                data = json.load(f)
            return [(user_id, torch.from_numpy(np.array(embedding)).to(self.device)) 
    ...
    def get_face_embedding(self, input_data):
        try:
            ...
            return embedding.cpu().numpy()[0]  # Return CPU array for compatibility
        except Exception as e:
        ...
        try:
            ...
            if boxes is not None:
                ...
                if face_tensors:
                    ...
                    with torch.no_grad():
                        embeddings = self.resnet(face_batch)  # Keep on GPU
                    
                    for i, (box, embedding) in enumerate(zip(valid_boxes, embeddings)):
                        best_match = None
    ...
    def calculate_similarity(self, embedding1, embedding2):
        ...
        emb1 = embedding1 / torch.norm(embedding1)
        emb2 = embedding2 / torch.norm(embedding2)
    ...
    if img_path:
        try:
            ...
            for user_id, stored_embedding in data.items():
                similarity = face_processor.calculate_similarity(
                    torch.from_numpy(current_embedding).to(face_processor.device),
                    torch.from_numpy(np.array(stored_embedding)).to(face_processor.device)
                )
