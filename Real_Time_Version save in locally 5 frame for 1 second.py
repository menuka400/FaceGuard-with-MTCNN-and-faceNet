...
def real_time_recognition(face_processor):
    ...
    # Variables for frame rate control (5 fps)
    frame_interval = 1.0 / 5.0  # 0.2 seconds between frames for 5 FPS
    last_frame_time = 0
    
    # Variables for database refresh
    ...
    while True:
        ...
        # Only process frame if enough time has passed (for 5 FPS)
        if time_elapsed >= frame_interval:
            ...
            # Check if it's time to refresh the face database
        ...
        # Check for quit key (more responsive than waiting for the next frame)
